              ...
              <table class="table table-bordered table-striped table-sm">
                ...
                <tbody>
                  <tr v-for="sale in sales" :key="sale.id">
                    ...
                    <button type="submit" class="btn btn-danger btn-sm" @click="changeState(sale.id)"
                        v-if="sale.state == 'register'">
                      <i class="fa fa-trash"></i>
                    </button>
                    </td>
                    <td v-text="sale.name"></td>
                    <td v-text="sale.type_voucher"></td>
                    <td v-text="sale.series_voucher"></td>
        ...
        <template v-else-if="list == 2">
          <div class="card-body">
            <div class="form-group row border">
              ...
              <div class="col-md-4">
                <div class="form-group">
                  <label for="client">Type Voucher</label>
            ...
            <div class="form-group row border">
              ...
              <div class="col-md-2">
                <div class="form-group">
                  ...
                  <input type="number" step="any" value="0" class="form-control" v-model="discount">
                </div>
              </div>
              <div class="col-md-2">
                <div class="form-group">
                ...
                <table class="table table-bordered table-striped table-sm">
                  ...
                  <tbody v-if="details.length">
                    <tr v-for="(detail, index) in details" :key="detail.id">
                      ...
                      <td>
                        <span style="color:red" v-show="detail.quantity>detail.stock">stock:{{detail.stock}}</span>
                        <input type="number" v-model="detail.quantity" class="form-control">
                      </td>
                      <td>
                        <span style="color:red" v-show="detail.discount>detail.price*detail.quantity">
                          Discount Exceeded!
                        </span>
                        <input type="number" v-model="detail.discount" class="form-control">
                      </td>
                      <td>{{ (detail.price * detail.quantity) - detail.discount }}</td>
                    </tr>
            ...
            <div class="form-group row">
              <div class="col-md-12">
                ...
                <button type="button" class="btn btn-primary" @click="registerSale()">Register</button>
              </div>
            </div>
          </div>
        </template>
        <template v-else-if="list == 3">
           <div class="card-body">
            <div class="form-group row border">
              <div class="col-md-9">
                <div class="form-group">
                  <label for="provider">Client</label>
                  <p v-text="client"></p>
                </div>
              </div>
              <div class="col-md-3">
...
export default {
  ...
  computed: {
    ...
    calculateTotal: function() {
      ...
      for (let i = 0; i < this.details.length; i++) {
        total = total + (this.details[i].price * this.details[i].quantity - this.details[i].discount);
      }
      return total;
    }
  },
  methods: {
    show(page, search, option) {
      let ctrl = this;
      let url =
        "/sale?page=" + page + "&search=" + search + "&option=" + option;
      axios
        .get(url)
        .then(function(res) {
          let response = res.data;
          ctrl.sales = response.sales.data;
    ...
    showArticles(searchArticles, optionArticles){
      let ctrl = this;
			let url = '/articles/sales?search=' + searchArticles + '&option=' + optionArticles;
      axios
        .get(url)
        .then(function(res) {
					let response = res.data;
    ...
    validateForm() {
      let ctrl = this;
      ctrl.cleanError();
      ctrl.details.map(function(item) {
        if (item.quantity > item.stock) {
          ctrl.listErr.push(`Stock Insufficient:  ${item.article}`);
        }
        if (item.discount > item.quantity * item.price) {
          ctrl.listErr.push(`Discount Exceeded:  ${item.article}`);
        }
      });
      if (ctrl.id_client == 0) ctrl.listErr.push("Select client");
      if (ctrl.type_voucher == 0) ctrl.listErr.push("Field required type voucher");
      if (ctrl.series_voucher == 0) ctrl.listErr.push("Field required series voucher");
      if (!ctrl.num_voucher) ctrl.listErr.push("Field required number voucher");
      if (!ctrl.tax) ctrl.listErr.push("Field required tax!");
      if (ctrl.details.length <= 0) ctrl.listErr.push("Field required details articels!");
      if (ctrl.listErr.length) ctrl.err = 1;
      return ctrl.err;
    },
    registerSale() {
    ...
    selectClients(search, loading) {
      ...
      var url = "/clients?filter=" + search;
      axios
        .get(url)
        .then(function(res) {
          let response = res.data;
          q: search;
          me.clients = response.clients;
          loading(false);
        })
    ...
    getArticles() {
      let me = this;
      let url = "/articles/search?filter=" + me.code;
      axios
        .get(url)
        .then(function(response) {
          ...
          if (me.articles.length > 0) {
            ...
            me.price = me.articles[0]["price"];
            me.stock = me.articles[0]["stock"];
          } else {
    ...
    showSaleDetail(sale){
      let ctrl = this;
      ctrl.list = 3;
      ctrl.client = sale.client;
      ctrl.type_voucher = sale.type_voucher;
      ctrl.series_voucher = sale.series_voucher;
      ctrl.num_voucher = sale.num_voucher;
      ctrl.discount = sale.discount;
      ctrl.tax = sale.tax;
      ctrl.total = sale.total;
      axios
        .get('/sale/detail?id=' + sale.id)
        .then(function(res) {
          let response = res.data;
